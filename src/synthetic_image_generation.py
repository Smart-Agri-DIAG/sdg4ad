...
def write_log(index, good_image_path, bad_image_path, log_folder):
    ...
    """
    with open(os.path.join(log_folder, f"log_anomaly_{index}.txt"), "w") as f:
...
def generate_synthetic_image(cfg, img_good, img_bad, mask_generator, num_anomalies=3):
    """
    ...
    # Sample good masks
    good_indices = random.choices(range(len(good_masks)), k=num_anomalies)
    good_masks = [good_masks[i].astype(np.uint8) for i in good_indices]

    # Sample bad masks
    bad_grapes = [img_bad * mask[:, :, None] for mask in bad_masks]
    bad_indices = get_indices_of_edgiest_grapes(cfg, bad_grapes, num_anomalies)
...
if __name__ == "__main__":
    ...
    for num_anomalies in [1, 3]:
        for cfg_path in cfg_paths:
            cfg = load_config(cfg_path)
            cfg['output_path'] = cfg['output_path'].replace("synthetic_images_PN", f"synthetic_images_PN_paste{num_anomalies}")
            print_config(cfg)
            set_seed(cfg['seed'])
            ...
            for index, bad_image_path in tqdm(enumerate(bad_image_paths), desc="Processing images", total=num_bad_images):
                ...
                new_img = generate_synthetic_image(cfg, img_good, img_bad, mask_generator, num_anomalies)
